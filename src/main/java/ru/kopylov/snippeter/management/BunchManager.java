...
public class BunchManager {
    ...
    public HashMap<Snippet, ArrayList<Feature>> fetchAllSnippets(Source source){
        HashMap<Snippet,ArrayList<Feature>> map = new HashMap<>();
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<SnippetFeatureBunch> query = cb.createQuery(SnippetFeatureBunch.class);
        Root<SnippetFeatureBunch> from = query.from(SnippetFeatureBunch.class);
        query.select(from);
        query.where(cb.equal(from.get("snippet").get("source"), source));
        em.createQuery(query).getResultStream().forEach(b -> treatOneBunch(b, map));
        return map;
    }

    private void treatOneBunch(SnippetFeatureBunch bunch, HashMap<Snippet,ArrayList<Feature>> map ){
